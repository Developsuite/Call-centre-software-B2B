...
export default function ProcessorSalesPage() {
 ...
 if (!isLoaded) {
 return (
 <DashboardLayout title="All Organization Sales">
 ...
 const tenantSales = sales.filter(s => {
 if (currentUser?.role === "SuperAdmin") return true;
 if (s.tenantId !== currentUser?.tenantId) return false;
 
 // For Admin, show all tenant sales
 if (currentUser?.role === "Admin") return true;

 // For Processors, show sales they are assigned to or have interacted with (in history)
 if (currentUser?.role === "Processor") {
 if (s.assignedTo === currentUser.name) return true;
 if (s.processorName === currentUser.name) return true;
 if (s.historyLogs && s.historyLogs.some(log => log.by === currentUser.name)) return true;
 return false; // Hide other processors' sales
 }

 return true; // Fallback
 });

 const dateFilteredSales = tenantSales
 .filter(sale => {
 ...
 .filter(sale => {
 const matchesSearch = sale.customer.toLowerCase().includes(searchQuery.toLowerCase()) || sale.id.toLowerCase().includes(searchQuery.toLowerCase()) || sale.agent.toLowerCase().includes(searchQuery.toLowerCase());
 ...
 <Card className="rounded-[1.5rem] border-none p-5 flex flex-col min-h-[calc(100vh-8rem)]">
 <div className="flex justify-between items-center mb-6">
 <h3 className="font-bold text-slate-800 dark:text-white text-lg">
 {currentUser.role === "Processor" ? "My Processed Sales" : "All Organization Sales"}
 </h3>
 ...
 <Input 
 type="text" 
 placeholder="Search ID, Customer, or Agent" 
 value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 className="pl-9 bg-slate-50 border-slate-200 dark:bg-slate-800 dark:border-slate-700 rounded-full h-9 text-sm w-48 focus-visible:ring-1 focus-visible:ring-[#ff5a36]" 
 />
 </div>
 ...
 className="bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700 rounded-full h-9 text-sm w-[140px] focus-visible:ring-1 focus-visible:ring-[#ff5a36]"
 />
 )}

 <DropdownMenu>
 <DropdownMenuTrigger className="rounded-full h-9 border px-4 text-sm font-medium flex items-center gap-2 cursor-pointer bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800 transition-colors">
 ...
 statusFilter === status 
 ? "bg-[#ff5a36] text-white border-[#ff5a36] shadow-[#ff5a36]/20" 
 ...
 <th className="pb-3 px-3 font-medium">Agent</th>
 ...
 <span className="hidden xl:inline text-xs uppercase tracking-wider">Process</span>
 </button>
 </Link>
 )}
