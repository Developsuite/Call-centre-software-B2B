  ...
  return (
    <DashboardLayout title={`Welcome, ${currentUser.name}`}>
      <div className="relative flex flex-col gap-5 font-sans max-w-[1200px] mx-auto w-full pb-10 min-h-screen overflow-x-hidden px-4 md:px-0">
        ...
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-gradient-to-br from-white/90 to-white/50 dark:from-slate-900/60 dark:to-slate-900/20 p-6 rounded-[1.5rem] border border-white/60 dark:border-slate-700/50 shadow-2xl shadow-[#ff5a36]/5 backdrop-blur-2xl relative overflow-hidden min-h-[120px] group transition-all duration-500 hover:shadow-[#ff5a36]/10">
          ...
          <div className="relative z-10 w-full sm:w-2/3">
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl md:text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-300 tracking-tight flex items-center gap-3">
                <div className="p-2 bg-gradient-to-br from-[#ff7a5c] to-[#ff5a36] rounded-xl shadow-lg shadow-[#ff5a36]/30">
          ...
          <div className="absolute right-0 bottom-0 top-0 h-full w-1/3 hidden sm:flex items-center justify-end pr-8 pointer-events-none">
             <img 
                ...
                alt="HR illustration" 
                className="h-[110%] w-auto object-contain drop-shadow-2xl transition-transform duration-500 group-hover:scale-105 group-hover:rotate-2" 
             />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card className="rounded-[1.5rem] bg-white/70 dark:bg-slate-900/40 backdrop-blur-2xl border border-slate-200/30 dark:border-slate-700/50 shadow-xl shadow-slate-200/20 dark:shadow-black/20 overflow-hidden flex flex-col transition-all hover:bg-white/90 dark:hover:bg-slate-800/60 duration-300">
              ...
              <div className="flex flex-col gap-1">
                {tenantUsers.slice(0, 4).map((user, i) => (
                  <div key={user.id} className="flex items-center justify-between p-3 hover:bg-white/50 dark:hover:bg-slate-800/50 rounded-[1rem] transition-colors cursor-pointer group">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <img 
                          ...
                          alt={user.full_name} 
                          className="w-10 h-10 rounded-full object-cover shadow-sm bg-slate-100 group-hover:shadow-md transition-shadow" 
                        />
                    ...
                    <div>
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold ${user.status === 'Active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-400'}`}>
                        {user.status}
                      </span>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card className="rounded-[1.5rem] bg-white/70 dark:bg-slate-900/40 backdrop-blur-2xl border border-slate-200/30 dark:border-slate-700/50 shadow-xl shadow-slate-200/20 dark:shadow-black/20 flex flex-col p-6 transition-all hover:bg-white/90 dark:hover:bg-slate-800/60 duration-300">
