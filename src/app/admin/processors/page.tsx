...
import { Search, Briefcase, ChevronRight, UserCircle, LayoutGrid, List } from "lucide-react"
import Link from "next/link"

export default function ProcessorsListPage() {
 ...
 return (
 <DashboardLayout title="All Processors">
 ...
 const processors = tenantUsers.filter(u => u.role === "Processor")
 ...
 <h1 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">All Processors</h1>
 </div>
 <p className="text-slate-500 text-sm">View and analyze performance of all processors</p>
 </div>
 ...
 className="pl-9 bg-white/70 dark:bg-slate-900/40 backdrop-blur-2xl border border-slate-200/30 dark:border-slate-700/50 shadow-xl shadow-slate-200/20 dark:shadow-black/20 hover:bg-white/90 dark:hover:bg-slate-800/60 hover:shadow-2xl transition-all duration-300 rounded-full h-9 text-sm w-48 sm:w-64 focus-visible:ring-1 focus-visible:ring-[#ff5a36]" 
 />
 </div>
 <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-full border ">
 <button
 onClick={() => setViewMode("card")}
 className={`p-1.5 rounded-full transition-colors ${viewMode === "card" ? "bg-white dark:bg-slate-700 text-[#ff5a36]" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"}`}
 title="Card View"
 >
 ...
 className={`p-1.5 rounded-full transition-colors ${viewMode === "list" ? "bg-white dark:bg-slate-700 text-[#ff5a36]" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"}`}
 title="List View"
 >
 ...
 const processorSales = tenantSales.filter(s => s.processorName === processor.name || s.processor_id === processor.id)
 const total = processorSales.length
 const connected = processorSales.filter(s => s.status === "Connected").length
 const rejected = processorSales.filter(s => s.status === "Rejected").length
 const convRate = total > 0 ? Math.round((connected / total) * 100) : 0

 return (
 <Link key={processor.id} href={`/admin/processors/${encodeURIComponent(processor.name)}`}>
 <Card className="rounded-[1.5rem] p-5 bg-white/70 dark:bg-slate-900/40 backdrop-blur-2xl border border-slate-200/30 dark:border-slate-700/50 shadow-xl shadow-slate-200/20 dark:shadow-black/20 hover:bg-white/90 dark:hover:bg-slate-800/60 hover:shadow-2xl transition-all duration-300 border-none hover:shadow-md transition-shadow cursor-pointer group flex flex-col h-full">
 <div className="flex items-start justify-between mb-4">
 <div className="flex items-center gap-3">
 {processor.avatarUrl ? (
 <img src={processor.avatarUrl} alt={processor.name} className="w-10 h-10 rounded-full object-cover" />
 ) : (
 ...
 <p className="text-xs text-slate-500 font-medium">Processor</p>
 </div>
 </div>
 ...
 <div className="grid grid-cols-3 gap-2 mt-auto pt-4 border-t border-slate-100 dark:border-slate-800/50">
 <div>
 ...
 <p className="font-bold text-lg text-emerald-500">{convRate}%</p>
 </div>
 <div>
 <p className="text-[10px] uppercase font-bold text-slate-400">Rejected</p>
 <p className="font-bold text-lg text-red-500">{rejected}</p>
 </div>
 </div>
 </Card>
 </Link>
 ...
 No processors found matching your search.
 </div>
 )}
 </div>
 ) : (
 ...
 <th className="py-4 px-6 font-bold">Processor</th>
 <th className="py-4 px-6 font-bold">Total Sales</th>
 <th className="py-4 px-6 font-bold">Conv. Rate</th>
 <th className="py-4 px-6 font-bold">Rejected</th>
 <th className="py-4 px-6"></th>
 </tr>
 </thead>
 <tbody>
 {filteredProcessors.map((processor, i) => {
 const processorSales = tenantSales.filter(s => s.processorName === processor.name || s.processor_id === processor.id)
 const total = processorSales.length
 const connected = processorSales.filter(s => s.status === "Connected").length
 const rejected = processorSales.filter(s => s.status === "Rejected").length
 ...
 return (
 <tr key={processor.id} className={`group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors ${!isLast ? 'border-b border-slate-50 dark:border-slate-800/50' : ''}`}>
 <td className="py-3 px-6">
 <div className="flex items-center gap-3">
 {processor.avatarUrl ? (
 <img src={processor.avatarUrl} alt={processor.name} className="w-8 h-8 rounded-full object-cover" />
 ) : (
 <UserCircle className="w-8 h-8 text-slate-300" />
 )}
 <div className="flex items-center gap-2">
 <span className="font-bold text-slate-800 dark:text-white">{processor.name}</span>
 {processor.isTeamLead && (
 <span className="bg-amber-100 text-amber-700 text-[9px] px-1.5 py-0.5 rounded-sm font-bold tracking-wider uppercase">Lead</span>
 )}
 </div>
 ...
 <div className="h-full bg-emerald-500" style={{ width: `${convRate}%` }}></div>
 </div>
 <span className="font-bold text-emerald-500 text-xs">{convRate}%</span>
 </div>
 </td>
 ...
 <Link href={`/admin/processors/${encodeURIComponent(processor.name)}`}>
